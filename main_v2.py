...
#     空気抵抗係数はkcdにて補正して使用
#
#     v2　着弾までの時間から空気抵抗係数を求める
#
#           2022/08/29      初速を補正計算し、弾道を再計算
#        v2 2022/09/14      初速位置補正を位置と時間の引き算だけに
#           2022/09/19      実験測定時刻の時に水平距離が一致する空気抵抗係数Cdを求める
#                           (空気抵抗係数は固定値)
#           2023/07/08      python v3.11でmatplotlibのimportができず3.9ではOK。 v3.11.4をインストールでOK。フォルダ？？？
#                           空気抵抗係数を求めない時、計算されないエラー
#
#           カムのらくがき帳
#
#  pythonへ移植
...
def muzzleVelocity(vLong, vLat):
    # 左右方向のブレ[m/sec]
    v0y = vLat
    v0x = math.sqrt(vLong ** 2 - v0y ** 2) * math.cos(math.pi *
                                                   elevAngle / 180)  # 水平方向の初速[m/sec]
    v0z = math.sqrt(vLong ** 2 - v0y ** 2) * math.sin(math.pi *
...
def rkf45(t, h, x, xe=9999, timeEnd=9999):
    ...
    # timeEnd:   時間終点値　使わない時　9999->到達しない
    #
    # 戻り値
    ...
    #      =  1  xe終点に到達し、精度調整中
    #      =  2  xe終点値に到達し計算完了
    #      =  3  timeEnd終了値に到達し、刻み幅調整
    #      =  4  timeEnd終了値に到達し計算完了
...
N = 12  # 微分方程式の階数
xVzero = np.array([x0, y0, z0, vx, vy, vz, omgx, omgy, omgz, ux, uy, uz])  # 微分方程式の解（初期値）を代入
xArray = np.copy(xVzero)
...
if CdFind == 1:
    ...
    print(f"#  測定距離:        {xTend:6.3f} m")
    print(f"#  測定着弾時間:  {tEnd:8.3f} msec")
...
if CdFind == 0:
    print("空気抵抗係数を求める計算ではない時は dando/main.pyを使用する")
    while 1:
        pass
...
while CdFind == 1:  # 空気抵抗係数を求める場合はループ
    ...
    ii = 0
    titleData()
    flightData(ii, tt, hh, xArray)  # t=0の時のデータ表示


    for ii in range(1, 999999):
        ans = 0
        tt, hh, xArray, ans = rkf45(tt, hh, xArray, xTarget[targetNum], tEnd)
        if ans < 0:
            print("t=", tt * 1000, "msec   error stop")
            while 1:
                pass
        ...
        # データを表示
        if ii % step == 0 or ans == 1 or ans == 3:
            # 表示周期毎と終点精度調整中に表示
            flightData(ii, tt, hh, xArray)
